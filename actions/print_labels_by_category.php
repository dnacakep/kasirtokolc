...
if (empty($categoryIds)) {
    redirect_with_message('/index.php?page=label_harga', 'Pilih setidaknya satu kategori yang ingin dicetak.', 'error');
}
...
$stmt = $pdo->prepare("
    SELECT p.id AS product_id, p.name AS product_name, p.barcode, b.sell_price, b.batch_code, c.name as category_name
    FROM products p
...
foreach ($labels as $label) {
    $labelsByCategory[$label['category_name']][] = $label;
}

$allLabels = [];
foreach ($labelsByCategory as $categoryName => $categoryLabels) {
    $allLabels = array_merge($allLabels, $categoryLabels);
}

$sheets = array_chunk($allLabels, 40);

inventory_log('label_printed', [
    'mode' => 'category_latest_multiple',
...
<head>
    <meta charset="utf-8">
    <title>Cetak Label Harga per Kategori</title>
    <style>
        :root {
            color-scheme: light;
        }
...
<body>
    ...
    <div class="sheet" data-sheet="<?= $sheetIndex + 1 ?>">
        <?php foreach ($allLabels as $label): ?>
            <?php
            if ($labelCount > 0 && $labelCount % 40 === 0) {
                echo '</div><div class="sheet" data-sheet="' . ($sheetIndex + 1) . '">';
                $sheetIndex++;
            }
            
            if ($currentCategory != $label['category_name']) {
                if ($labelCount > 0 && $labelCount % 40 !== 0) {
                    // This is a simplistic way to handle category breaks
                    // A more robust solution might be needed for complex layouts
                }
                $currentCategory = $label['category_name'];
            }
            ?>
            <div class="label-card">
                <header>
                    <span class="store-name"><?= sanitize($store['store_name']) ?></span>
                    <span class="sheet-number">#<?= $sheetIndex + 1 ?></span>
                </header>
        ...
        <?php
        $remainingSlots = 40 - ($labelCount % 40);
        if ($remainingSlots < 40) {
            for ($i = 0; $i < $remainingSlots; $i++) {
                echo '<div class="label-card"></div>';
            }
        }
        ?>
    </div>
